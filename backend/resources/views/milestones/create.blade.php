...
<head>
    ...
    <title>Create Milestone - OPF-CD</title>
...
<body class="min-h-full bg-slate-950 text-gray-100">
    <div class="min-h-screen p-8" x-data="milestoneCreate({{ $projectId }})">
        <!-- Header -->
        <div class="max-w-3xl mx-auto mb-8">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-3xl font-bold">Create New Milestone</h1>
                    <p class="text-gray-400 mt-1">Add a new payment milestone to the project</p>
                </div>
                <a :href="'/projects/' + projectId + '/milestones'" class="px-4 py-2 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 transition-all">
                    Cancel
                </a>
            </div>
        </div>
        ...
        <div class="max-w-3xl mx-auto">
            <form @submit.prevent="submitMilestone" class="rounded-xl bg-white/5 backdrop-blur-xl border border-white/10 p-8 space-y-6">
                <!-- Milestone Name -->
                <div>
                    <label class="block text-sm font-medium text-gray-300 mb-2">Milestone Name *</label>
                    <input type="text" x-model="form.name" required placeholder="e.g., Initial Payment, Project Completion"
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-amber-400">⚠️ Note: Paid milestones cannot be edited or deleted.</p>
                ...
                <div class="flex justify-end gap-4">
                    <a :href="'/projects/' + projectId + '/milestones'" class="px-6 py-2 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 transition-all">
                        Cancel
                    </a>
                    <button type="submit" :disabled="submitting"
                        ...
                        <span x-show="!submitting">Create Milestone</span>
    ...
    <script>
        function milestoneCreate(projectId) {
            return {
                projectId: projectId,
                form: {
                    name: '',
                ...
                async submitMilestone() {
                    ...
                    try {
                        ...
                        if (response.ok && data.success) {
                            ...
                            this.errorMessage = data.message || 'Failed to create milestone.';
                        }
                    } catch (e) {
                        this.errorMessage = 'Error creating milestone: ' + e.message;
                    } finally {
