        ...
        $opportunities = DB::table('opportunities')
            ...
            ->map(function ($opportunity) {
                return [
                    ...
                    'qualification_score' => (int) ($opportunity->qualification_score ?? 0),
                    'budget_confirmed' => $opportunity->budget_confirmed ?? 'unknown',
                    'authority_level' => $opportunity->authority_level ?? 'unknown',
                    'need_validation' => $opportunity->need_validation ?? 'unknown',
                    'timeline_urgency' => $opportunity->timeline_urgency ?? 'unclear',
                    'strategic_fit' => $opportunity->strategic_fit ?? 'cold_lead',
                    'disqualification_reason' => $opportunity->disqualification_reason,
    ...
    public function createOpportunity(array $data, int $userId, ?Request $request = null): array
    {
        try {
            $opportunityId = DB::table('opportunities')->insertGetId([
                ...
                'budget_confirmed' => $data['budget_confirmed'] ?? 'unknown',
                'authority_level' => $data['authority_level'] ?? 'unknown',
                'need_validation' => $data['need_validation'] ?? 'unknown',
                'timeline_urgency' => $data['timeline_urgency'] ?? 'unclear',
                'strategic_fit' => $data['strategic_fit'] ?? 'cold_lead',
    ...
    public function updateOpportunity(int $opportunityId, array $data, int $userId, ?Request $request = null): array
    {
        ...
        try {
            $updateData = array_filter([
                ...
                // BANT lead qualification fields
                'budget_confirmed' => $data['budget_confirmed'] ?? null,
                'authority_level' => $data['authority_level'] ?? null,
                'need_validation' => $data['need_validation'] ?? null,
                'timeline_urgency' => $data['timeline_urgency'] ?? null,
                'strategic_fit' => $data['strategic_fit'] ?? null,
                'disqualification_reason' => array_key_exists('disqualification_reason', $data) ? $data['disqualification_reason'] : null,
                'last_contact_date' => array_key_exists('last_contact_date', $data) ? $data['last_contact_date'] : null,
                'updated_at' => now(),
            ], function ($value) {
                return $value !== null;
            });
